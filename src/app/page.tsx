...
export default function WeddingInvitation() {
  ...
  // --- DATA UNDANGAN (SUDAH DISESUAIKAN) ---
  const groomName = "Irfan";
  const brideName = "Afra";
  const weddingDate = "Sabtu, 14 Februari 2026";
  const targetDate = "2026-02-14T09:00:00+07:00";
  const akadTime = "09:00 - 10:00 WIB";
  const resepsiTime = "11:00 - 14:00 WIB";
   const locationName = "Kediaman Mempelai Wanita"; 
  const locationAddress = "Jl. Delima Gg. Ikhlas No. 25, Tampan, Pekanbaru, Riau"; 
  const mapsUrl = "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d15958.64817478619!2d101.38587604473335!3d0.4950392393322128!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x31d5a85536b2f693%3A0x1b79e262a4858e3!2sJl.%20Delima%2C%20Kota%20Pekanbaru%2C%20Riau!5e0!3m2!1sid!2sid!4v1689554658569!5m2!1sid!2sid"; 
  const accountNumber = "1234567890";
  const bankName = "BCA";
  const accountName = "Irfan & Afra"; // Disesuaikan
  const waConfirmation = "6281298765432";

  useEffect(() => {
    document.title = `The Wedding of ${groomName} & ${brideName}`;
  ...
  }, [targetDate, groomName, brideName]);
  ...
  const handleOpenInvitation = () => {
    ...
    if (audioRef.current) {
        audioRef.current.play().catch(error => console.log("Autoplay ditolak oleh browser:", error));
    }
  };
  ...
  return (
    <>
      <audio ref={audioRef} loop src="/audio/wedding.mp3" />
        ...
        {!isInvitationOpen && (
          ...
          >
            <div className="text-center space-y-6 flex flex-col items-center">
              ...
              <h1 className={`${dancing.className} text-6xl md:text-8xl text-amber-50`}>{groomName} & {brideName}</h1>
              <p className={`${lato.className} text-lg text-gray-300`}>{weddingDate}</p>
              <motion.button
      ...
      <main className={`${lato.className} min-h-screen bg-[#fdfcf9] text-[#5c5c5c] antialiased transition-opacity duration-1000 ${isInvitationOpen ? 'opacity-100' : 'opacity-0'}`}>
        <div className="max-w-2xl mx-auto">
          ...
          >
            ...
            <motion.h1 variants={itemVariants} className={`${dancing.className} text-7xl md:text-8xl text-[#bda08c]`}>{groomName} & {brideName}</motion.h1>
            <motion.p variants={itemVariants} className="text-lg font-bold">{weddingDate}</motion.p>
          ...
          >
            ...
            <div className="flex flex-col md:flex-row justify-center items-center gap-10 md:gap-16">
              <motion.div variants={itemVariants} className="space-y-4">
                <h2 className={`${dancing.className} text-6xl text-[#bda08c]`}>{groomName}</h2>
                <p className="text-sm">Putra dari Bapak Sudirman & Ibu Fatimah</p>
              </motion.div>
              <motion.span variants={itemVariants} className="text-6xl font-thin text-[#bda08c]">&</motion.span>
              <motion.div variants={itemVariants} className="space-y-4">
                <h2 className={`${dancing.className} text-6xl text-[#bda08c]`}>{brideName}</h2>
                <p className="text-sm">Putri dari Bapak Anton & Ibu Ani</p>
              </motion.div>
            </div>
          </motion.section>
          ...
          >
            ...
            <motion.p variants={itemVariants} className="mb-12 text-lg">{weddingDate}</motion.p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-xl mx-auto">
              <motion.div variants={itemVariants} className="bg-[#fdfcf9] p-8 rounded-lg shadow-md border border-gray-200/80">
                ...
                <p className="mt-2">{akadTime}</p>
              </motion.div>
              <motion.div variants={itemVariants} className="bg-[#fdfcf9] p-8 rounded-lg shadow-md border border-gray-200/80">
                <PartyPopper className="mx-auto mb-4 text-[#bda08c]" size={40} />
                <h4 className={`${cormorant.className} text-2xl font-semibold`}>Resepsi</h4>
                <p className="mt-2">{resepsiTime}</p>
              </motion.div>
            </div>

            <motion.div variants={itemVariants} className="mt-12">
                <MapPin className="mx-auto mb-4 text-[#bda08c]" size={40} />
                <h4 className={`${cormorant.className} text-2xl font-semibold`}>{locationName}</h4>
                <p className="mt-2 max-w-md mx-auto">{locationAddress}</p>
          ...
          >
            ...
            <motion.div variants={itemVariants} className="p-6 bg-white rounded-lg shadow-md border border-gray-200/80 max-w-sm mx-auto">
                ...
                <p className="text-2xl my-2 tracking-widest text-[#bda08c]">{accountNumber}</p>
                <p className="text-sm">a.n. {accountName}</p>
                <button 
                  onClick={handleCopy}
            ...
            <motion.p variants={itemVariants} className="mt-6 text-sm">
             ...
             <motion.a 
                ...
                href={`https://wa.me/${waConfirmation}?text=Halo%20${groomName}%20%26%20${brideName},%20saya%20ingin%20mengirimkan%20hadiah%20pernikahan.`}
                target="_blank" 
          ...
          <section className="py-16 md:py-24 px-6 text-center bg-white shadow-inner">
            ...
            <p className="mt-2 mb-12">Wassalamu’alaikum Warahmatullahi Wabarakatuh</p>
            <h3 className={`${dancing.className} text-7xl md:text-8xl text-[#bda08c]`}>{groomName} & {brideName}</h3>
          </section>

          {/* --- Footer --- */}
          <footer className="text-center py-6 px-4 text-xs text-gray-400">
            <p>© {new Date().getFullYear()} {groomName} & {brideName}. Dibuat dengan ❤️.</p>
          </footer>
        </div>
      </main>
    </>
